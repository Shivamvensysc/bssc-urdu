...
// /**
//  * ── BUG FIX ────────────────────────────────────────────────────────
//  * `data.category` and `data.officerType` hold the human-readable *label*
//  * returned by the /categories and /public/type-of-ex-officer APIs (e.g.
//  * "Unreserved (General)", "Commissioned Officer"). The age-eligibility
//  * engine (`validateAgeEligibility` / `getBaseMaxAge`) — and several checks
//  * in this file — compare against short *codes* ("UR", "BC", "SC", "ST",
//  * "EBC", "EWS", "OTHER_RANKS", "COMMISSIONED_OFFICER", "ECO", "SSCO").
//  * Because a label never equals a code, every one of those comparisons was
//  * silently failing, which is why age-eligibility / category-certificate /
//  * transgender-category validation appeared "not to work".
//  *
//  * These two mapping helpers translate the API label back to the short
//  * code so the rest of the validation logic works as designed.
//  *
//  * TODO: ask the backend to add a stable `code` field to /categories and
//  * /public/type-of-ex-officer so this heuristic can be removed. Until then,
...
//    *
//    * NOTE: `data.category` / `data.officerType` hold API *labels*, so they
//    * are translated to short codes via mapCategoryLabelToCode /
//    * mapOfficerLabelToCode before being handed to validateAgeEligibility —
//    * see the bug-fix comment near those helpers above.
//    */
...
//   const showScribeField = data.isBiharDomicile === "YES" && data.isPwD === "YES";
...
const REQUIRED_BY_SECTION: Record<SectionId, (keyof FormData)[]> = {
  ...
  contact: [
    ...
    "emailId",
    "confirmEmailId",
    // "captchaInput", // comment for testing 
    // NEW — password is now part of the form itself, validated and
    // counted toward section completion exactly like every other
    // required contact field.
    "password",
